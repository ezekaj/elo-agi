...
class BenchmarkCrusher:
    ...
    def __init__(self, model: str = "ministral-3:8b"):
        ...
        self.tests = [
            ...
            {"question": "If a rectangle has length 8 and width 5, what is its area?", "answer": "40", "keywords": ["40"], "category": "math"},
            {"question": "All cats are mammals. All mammals are animals. Is a cat an animal? Answer yes or no.", "answer": "yes", "keywords": ["yes", "mammal", "animal"], "category": "logic"},
            {"question": "If it rains, the ground gets wet. The ground is wet. Can we conclude it rained? Answer yes or no.", "answer": "no", "keywords": ["no", "not necessarily"], "category": "logic"},
            {"question": "A person puts ice cream in the oven at 400 degrees. What happens?", "answer": "melts", "keywords": ["melt", "liquid"], "category": "common_sense"},
            {"question": "A farmer has 17 sheep. All but 9 run away. How many sheep does he have left?", "answer": "9", "keywords": ["9"], "category": "trick"},
            {"question": "If you have 3 apples and you take away 2, how many apples do YOU have?", "answer": "2", "keywords": ["2"], "category": "trick"},
            ...
            {"question": "Sally puts a marble in her basket and leaves. Anne moves it to her box. Where will Sally LOOK for the marble?", "answer": "basket", "keywords": ["basket"], "category": "theory_of_mind"},
        ]
    ...
    def learn_unique_fact(self) -> bool:
        ...
        for item in items[:3]:
            ...
            if self.evolution.mark_learned(analyzed.get('summary', '')):
                self.trainer.learn(analyzed.get('topic', title), analyzed.get('knowledge', content[:1000]), source)
                self._save_training_data(analyzed, source)

                stats = self.evolution.get_stats()
                print(f"[LEARN {stats['facts_this_cycle']}/100] [{source}] {analyzed.get('topic', title)[:50]}")
                return True
        ...
        if source in ['math', 'logic']:
            ...
            try:
                if self.weak_areas:
                    ...
                    query = random.choice(['machine learning basics', 'logic puzzles', 'math problem solving'])
            ...
            try:
                topic = random.choice(['Artificial intelligence', 'Machine learning', 'Logic', 'Mathematics', 'Reasoning'])
    ...
    def _benchmark_qa(self, category: str) -> List[Dict]:
        ...
        qa = {
            'math': [
                ("5 apples for $2 each, pay with $20, how much change?", "5×$2=$10. Change: $20-$10=$10. Answer: 10"),
                ("Train at 60mph for 2.5 hours, how far?", "Distance=Speed×Time=60×2.5=150. Answer: 150 miles"),
                ("Rectangle length 8, width 5, what's area?", "Area=8×5=40. Answer: 40"),
                ("What is 15% of 80?", "15%×80=0.15×80=12. Answer: 12"),
            ],
            'logic': [
                ("All cats are mammals. All mammals are animals. Is a cat an animal?", "Syllogism: cats→mammals→animals. YES, cats are animals."),
                ("If rain then wet ground. Ground is wet. Did it rain?", "Fallacy of affirming consequent. Could be sprinklers. Answer: NO, not necessarily."),
                ("If A implies B and B is false, what about A?", "Modus tollens: A→B, ¬B, therefore ¬A. Answer: A is false."),
            ],
        }
        ...
        try:
            for qa in analyzed.get('qa_pairs', []):
                if qa.get('q') and qa.get('a'):
                    pair = {"prompt": qa['q'], "completion": qa['a'], "source": source, "timestamp": datetime.now().isoformat()}
                    with open(training_file, 'a') as f:
                        f.write(json.dumps(pair) + '\n')

            for fact in analyzed.get('facts', []):
                if fact:
                    pair = {"prompt": f"What about {analyzed.get('topic', 'this')}?", "completion": fact, "source": source, "timestamp": datetime.now().isoformat()}
                    with open(training_file, 'a') as f:
                        f.write(json.dumps(pair) + '\n')
        except Exception:
            pass
