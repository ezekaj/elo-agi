...
class MetaLearner:
    ...
    def __init__(self, params: Optional[MetaParams] = None):
        ...
        self._strategy_weights: Dict[str, float] = {}  # Must be initialized before _initialize_strategies
    ...
    def _update_strategy_weights(self) -> None:
        ...
        for strategy_id, strategy in self._strategies.items():
            ...
            self._strategy_weights[strategy_id] = (
                (1 - self.params.learning_rate) * old_weight +
                self.params.learning_rate * score
            )
    ...
    def record_performance(self, performance: float) -> None:
        ...
        if len(self._performance_history) >= 2:
            ...
            if times[-1] != times[0]:
                self._improvement_velocity = (
                    (perfs[-1] - perfs[0]) / (times[-1] - times[0])
                )
    ...
    def should_improve(self) -> Tuple[bool, str]:
        ...
        recent_mods = [
            m for m, r in self._experience[-10:]
    ...
    def recommend_magnitude(self) -> float:
        ...
        recent_successful = [
            m for m, r in self._experience[-50:]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        successful = [
            (m, r) for m, r in self._experience
            if r.status == UpdateStatus.APPLIED
        ]
        ...
        return {
            ...
            'current_performance': self._performance_history[-1][1] if self._performance_history else 0.0,
        }
